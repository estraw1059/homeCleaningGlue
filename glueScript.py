...
print(credentials.token)  # Trigger the token fetch
...
normalized_rows = [row + [None] * (expected_len - len(row)) for row in data[1:]]
data = pd.DataFrame(normalized_rows, columns=data[0])
print(data.head())
bucket = args['bucket']
folder = args['folder']
